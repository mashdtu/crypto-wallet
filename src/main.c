...
static int cmd_settings(void)
{
    ...
    if (!f)
    {
        fprintf(stderr, "error: could not save settings\n");
        return 1;
    }
    fprintf(f, "%s\n", selected);
    fclose(f);
...
static int cmd_balance(uint32_t gap_limit)
{
    ...
    int have_price = (strcmp(currency, "NONE") != 0) && (network_get_btc_price(currency, &btc_price) == 0);
                    ...
                    uint32_t from_index)
{
    ...
    if (dest_witver < 0 || (dest_prog_len != 20 && dest_prog_len != 32) || (dest_witver == 0 && dest_prog_len != 20) || (dest_witver == 1 && dest_prog_len != 32))
    {
...
int main(int argc, char *argv[])
{
    ...
    if (strcmp(argv[argi], "balance") == 0)
    {
        ...
        if (argc > argi + 1)
            gap = (uint32_t)strtoul(argv[argi + 1], NULL, 10);
        if (gap == 0)
            gap = GAP_LIMIT;
